...
const LearningForm = ({ courseLearning, setCourseLearning, id }) => {
    ...
    const handleSave = async () => {
        try {
            const data = await updateCourseDetails(id, { learnings: input });
            setCourseLearning(data.learnings);
        ...
        <div className="bg-secondary rounded-md p-3">
            ...
            <div>
                {isEditing ? (
                    ...
                    <>
                        {courseLearning.length ? (
                            courseLearning.map((learn) => <p>{learn}</p>)
                        ) : (
                            // {courseLearning.map((learning)=><p>{learning}</p>)}
                            <p className="text-sm text-muted-foreground italic">
                                No learning provided
                            </p>
                        )}
                    </>
                )}
